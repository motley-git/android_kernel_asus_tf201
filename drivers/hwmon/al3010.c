				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	if(light_sensor_ready == false){
		...
		if( tmp_lux > 0){
			...
			catch_first_poll_time = true;
			pr_debug("light sensor debug : al3010 first poll timestamp , tv_sec = %d\n",t_first_poll_time.tv_sec);
			pr_debug("light sensor debug : al3010 first poll timestamp , tv_usec = %d\n",t_first_poll_time.tv_usec);
			return sprintf(buf, "%d\n", -1);
		}else{
			...
			pr_debug("light sensor debug : al3010 current timestamp , tv_sec = %d\n",t_current_time.tv_sec);
			pr_debug("light sensor debug : al3010 current timestamp , tv_usec = %d\n",t_current_time.tv_usec);
