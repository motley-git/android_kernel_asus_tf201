...
static void battery_status_poll(struct work_struct *work)
{
	...
	if(!battery_driver_ready)
		pr_debug("battery_status_poll:driver not ready\n");
...
void setup_detect_irq(void )
{
	...
	if (ret < 0) {
		pr_debug("request battery_detect gpio failed\n");
	...
	if (pad_device->irq_battery_detect < 0) {
		pr_debug("invalid battery_detect GPIO\n");
		pad_device->gpio_battery_detect = -1;
	...
	if (ret < 0) {
			pr_debug("failed to configure GPIO\n");
	...
	if (ret < 0) {
            pr_debug("failed to request  battery_detect irq\n");
	}
	 pad_device->battery_present=!(gpio_get_value(pad_device->gpio_battery_detect));
...
void setup_low_battery_irq(void )
{
	...
	if (ret < 0) {
		pr_debug("request low_battery_detect gpio failed\n");
	...
	if (pad_device->irq_low_battery_detect< 0) {
		pr_debug("invalid low_battery_detect gpio\n");
		pad_device->gpio_low_battery_detect = -1;
	...
	if (ret < 0) {
			pr_debug("failed to configure low_battery_detect  gpio\n");
			gpio_free(pad_device->gpio_battery_detect);
			pad_device->gpio_low_battery_detect= -1;
			goto exit;
	}
	ret = request_irq(pad_device->irq_low_battery_detect , low_battery_detect_isr,
       ...
       pad_device->low_battery_present=gpio_get_value(pad_device->gpio_low_battery_detect);
	if (ret < 0) {
            pr_debug("failed to request  low_battery_detect irq\n");
	}
...
void   init_docking_charging_irq(void)
{
	...
	if (rc< 0)
		pr_debug(KERN_ERR"TEGRA_GPIO_PS5 GPIO%d request fault!%d\n",TEGRA_GPIO_PS5,rc);
	rc= gpio_direction_input(TEGRA_GPIO_PS5);
	if (rc)
	        pr_debug(KERN_ERR"gpio_direction_input failed for input TEGRA_GPIO_PS5=%d\n", TEGRA_GPIO_PS5);
	...
	pad_device->dock_charger_pad_interrupt_enabled=true;
	if (rc < 0)
	        pr_debug(KERN_ERR"Could not register for TEGRA_GPIO_PS5 interrupt, irq = %d, rc = %d\n", gpio_to_irq(TEGRA_GPIO_PS5), rc);
}
	...
	union power_supply_propval *val)
{
	...
	if (pad_device->smbus_status < 0) {
		...
		if(psp == POWER_SUPPLY_PROP_TEMP && (++pad_device->temp_err<=3) &&(pad_device->old_temperature!=0xFF)){
			...
			pr_debug("read battery's tempurate fail use old temperature=%u pad_device->temp_err=%u\n",val->intval,pad_device->temp_err);
			return 0;
		}
		else
		return -EINVAL;
	...
	if (psp == POWER_SUPPLY_PROP_STATUS) {
		...
		if ( (ret/10) >= MAXIMAL_VALID_BATTERY_TEMP && (pad_device->temp_err ==0)){
			ret=300;
			pr_debug("[Warning] pad_get_psp  batttery temp=%u, set to 30.0\n",ret);
			WARN_ON(1);
			pad_device->temp_err++;
		}
		else
		...
		pr_debug("pad_get_psp  batttery temperature=%u\n",val->intval );
	}
	return 0;
}
static int pad_get_capacity(union power_supply_propval *val)
{
	...
	if (pad_device->smbus_status < 0) {
		...
		return -EINVAL;
			...
			pad_device->cap_err++;
			pr_debug("read capacity fail, use old capacity=%u pad_device->cap_err=%u\n",val->intval,pad_device->cap_err);
			return 0;
		}
	}
	...
	const struct i2c_device_id *id)
{
	...
	for (i = 0; i < ARRAY_SIZE(pad_supply); i++) {
		...
		if (rc) {
			pr_debug(KERN_ERR "Failed to register power supply\n");
			while (i--)
	...
	battery_cable_status = get_usb_cable_status();
	 ...
	 pr_debug(KERN_INFO "battery register misc device for I2C stress test rc=%x\n", rc);
